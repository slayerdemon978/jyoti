...
class PDFOutlineExtractor:
    ...
    def is_likely_heading(self, line: str, page_num: int = 0, all_lines: List[str] = None) -> bool:
        ...
        skip_patterns = [
            ...
            r'mission statement:', r'goals:', r'•',  # Skip bullet points and mission statements
            r'^software testing$', r'^foundation level extensions$', r'^qualifications board$'  # Skip header/footer text
        ]
        ...
        # PRIORITY 6: All caps text (likely headings in many document types)
        # Only apply for non-technical documents (those without numbered sections)
        has_numbered_sections = any(re.match(r'^\d+\.', l.strip()) for l in (all_lines or []))
        if (not has_numbered_sections and 
            line_clean.isupper() and 
            len(line_clean) > 3 and len(line_clean) < 60 and
            not re.search(r'\d{4}', line_clean) and  # Not years
            'COPYRIGHT' not in line_clean):
            return True
        
        # PRIORITY 7: Title case standalone text (potential headings)
        # Only apply for non-technical documents
        words = line_clean.replace('-', ' ').split()  # Handle hyphenated words
        if (not has_numbered_sections and
            len(words) >= 2 and len(words) <= 8 and  # Allow slightly longer for hyphenated titles
            len(line_clean) < 80 and  # Longer max length for titles
            all(word[0].isupper() or word.lower() in ['and', 'or', 'of', 'the', 'in', 'to', 'for', 'with', 'on', 'at', 'by'] for word in words if word) and
            not line_clean.endswith('.') and
            not any(char in line_clean for char in ['(', ')', '"', '"', '"', ',']) and  # Not quotes, parentheses, or commas
            not re.search(r'\d{4}', line_clean) and  # Not containing years
            not any(word.lower() in ['students', 'student', 'provide', 'opportunity', 'experience', 'program', 'course'] for word in words)):  # Skip common descriptive words
            return True
        
        # PRIORITY 8: Text with exclamation marks (could be headings)
...
def main():
    ...
    # Use input and output directories
    ...
    for pdf_file in pdf_files:
        try:
            ...
            with open(output_file, "w", encoding='utf-8') as f:
                json.dump(result, f, indent=4, ensure_ascii=False)  # Use 4-space indent to match expected format
